...
- 권장 브랜치 예시: `main` (또는 Pages 전용 브랜치 `gh-pages`)
- `_config.yml`의 `theme: minima`는 GitHub Pages에서 일반적으로 지원되는 기본 Jekyll 테마입니다.

자동 배포가 필요하면 아래 워크플로를 사용하세요.

- 워크플로우 파일: `.github/workflows/jekyll-deploy-branch.yml`
- 기본 동작: `main` 푸시 시 Jekyll 빌드 후 `gh-pages` 브랜치로 배포
- 수동 실행: Actions > **Jekyll Build and Deploy Branch** > Run workflow
  - `deploy_branch` 입력값으로 원하는 신규 배포 브랜치를 지정 가능
