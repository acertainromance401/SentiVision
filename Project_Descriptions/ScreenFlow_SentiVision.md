...
| 화면 ID | 화면 이름 | 현재 상태 | 역할 |
|---|---|---|---|
| SC-0 | 초기 세팅 | 구현 | 프로필, 기준 감정·색상, 체감 강도 저장 |
| SC-A | 환영 | 대체 구현 | 별도 홈 대신 프로필 배너와 탭 진입 사용 |
| SC-B | 캔버스 | 구현/부분 | PencilKit, 펜·지우개·기본 팔레트·굵기; 고급 색상 도구는 백로그 |
| SC-C | 분석 로딩 | 백로그 | 현재 분석은 동기식 온디바이스 처리 |
| SC-D | 감정 전시 | 구현 | 감정, 해석, 점수, 대표색, 패밀리, 3D 분포 표시 |
| SC-E | 피드백 | 백로그 | 감정 수정과 메모 입력 미구현 |
| SC-F | 아카이브 | 구현/부분 | 로컬 카드 저장·목록·결과 재열람; 작품 이미지·편집은 백로그 |
| SC-ERR | 오류 처리 | 부분 | 빈 캔버스 분석 알림 구현; 네트워크 재시도는 원격 연동 시 후속 |

현재 동작 플로우는 `초기 세팅 → 탭 화면 → 캔버스 → 온디바이스 분석 → 감정 전시 → 로컬 저장 → 아카이브 재열람`이다. 아래 상세 명세는 구현된 항목과 목표 UX를 함께 유지하며, API 호출·피드백 관련 항목은 후속 요구사항이다.
...
| 요소              | 유형        | 내용                                     |
|------------------|------------|------------------------------------------|
| 프로필 이름      | 텍스트 필드 | 개인 식별용 이름 입력                    |
| 기준 감정        | 드롭다운    | CALMNESS, TRANQUILITY 등 선택            |
| 기준 색상        | 색상 선택기 | 개인 기준을 대표하는 색상 선택           |
| 체감 조절        | 슬라이더    | 해석 강도/민감도 조정                    |
| 시작하기 버튼    | 주요 버튼   | 설정 저장 후 SC-A로 이동                 |
...
| 요소                | 유형          | 내용                                         |
|--------------------|--------------|----------------------------------------------|
| 도구 바             | 툴바          | `브러시`, `지우개`, `색상휠`, `HEX`, `RGB`, `스포이트` |
| 드로잉 영역         | 캔버스 뷰     | 자유 드로잉 가능, 터치/마우스 입력 지원       |
| 팔레트 미리보기     | 색상 스와치   | 현재 그림의 선택/추출 색상을 실시간 표시 |
...
| 요소               | 유형           | 내용                                           |
|-------------------|---------------|------------------------------------------------|
| 감정 제목 레이블   | 강조 텍스트    | `감정 제목: {emotional_title}`                 |
| 해석 문장 레이블   | 본문 텍스트     | `해석: {interpretation}`                       |
| 점수 분포 카드     | 리스트/그래프  | 상위 감정 최대 5개, 각 감정명과 신뢰도 점수     |
| 대표 색상 스와치   | 색상 칩        | 분석에 사용된 대표 색상 최대 4개               |
| 전시 저장 버튼     | 주요 버튼      | 피드백 제출 없이 아카이브에 저장               |
...
| 사용자 액션      | 전환 대상 | 조건                          |
|-----------------|-----------|-------------------------------|
| `전시 저장` 탭  | SC-F      | 피드백 저장 성공               |
| `전시 저장` 탭  | SC-ERR    | 저장 실패                      |
...
| 요소            | 유형         | 내용                                                 |
|----------------|-------------|------------------------------------------------------|
| 분석 로그 목록  | 스크롤 리스트 | 날짜/시각, 감정 제목, 수정 감정(없으면 `수정 없음`), 대표 색상 |
| 상세보기 버튼   | 텍스트 버튼  | 로그 항목별 제공, 팔레트 및 메모 상세 확인            |
| 홈으로 이동     | 뒤로 가기   | SC-A로 복귀                                           |

- 로그는 최신 순으로 정렬한다.
- 데이터 없을 경우 `아직 전시된 기록이 없습니다.` 빈 상태(empty state) 메시지를 표시한다.
...
SC-0 → 탭 컨테이너
    ├── SC-D/3D 분포
    ├── SC-B 캔버스 → 로컬 분석 → SC-D → 전시 저장 → SC-F
    └── SC-F 아카이브 → 저장 결과 선택 → SC-D
```
...
| 앱 | 중심 화면 | 역할 |
|---|---|---|
| iPad 제작 앱 | SC-A~SC-F | 그림 제작, 분석, 전시 저장, 아카이브 관리 |
| iPhone 감상 앱 | 아카이브, 상세 보기 | 무료 감상용 동반 앱 |
```
...
```mermaid
flowchart TD
    Z[SC-0 초기 세팅] --> A[SC-A 홈]
    A -->|새 그림 시작| B[SC-B 캔버스]
...
| 문서                          | 버전  | 관계                               |
|------------------------------|-------|------------------------------------|
| PRD_SentiVision.md           | v1.1  | 기능 요구사항(FR-1~FR-6) 기준      |
