...
이 문서는 목표 요구사항과 현재 구현을 함께 관리한다. 상태 표에서 **구현**은 앱의 기본 동작 경로가 연결된 기능, **프로토타입**은 검증 가능하지만 제품 요구 일부가 남은 기능, **백로그**는 설계 또는 계획 단계 기능을 뜻한다.
...
| 요구사항 | 상태 | 2026-09-02 기준 |
|---|---|---|
| FR-1 드로잉 | 구현 | PencilKit 캔버스, 펜/지우개, 기본 팔레트, 굵기 조절 |
| FR-1-1 고급 색상 선택 | 백로그 | 색상 휠, HEX/RGB 입력, 프리셋 저장, 스포이트는 미구현 |
| FR-1-2 초기 개인화 | 프로토타입 | 프로필·기준 감정·기준 색상·체감 강도 저장; 실제 분석 가중치 반영은 후속 |
| FR-2 분석 | 구현/프로토타입 | 앱은 온디바이스 분석 구현. Node `/analyze`는 팔레트 JSON 휴리스틱이며 앱 미연동 |
| FR-3 피드백 | 백로그 | 앱 수정 UI와 `POST /feedback` 미구현 |
| FR-4 결과 시각화 | 구현 | 해석, 신뢰도, 점수, 대표색, 감정 패밀리, 3D 분포 표시 |
| FR-5 분석 파이프라인 | 구현/연구 | 앱은 전경 픽셀+K-means+최근접 표본, Python은 Saliency+K-means+KNN 검증 |
| FR-6 데이터 품질 | 프로토타입 | 앱 라벨 정규화·오탈자 매핑, Python 데이터 점검 자산 존재 |
...
## 9. 현재 기술/시스템 범위
- iPad App: SwiftUI, PencilKit, SceneKit, UIKit (실기기 데모 구현)
- Companion: iPhone 감상용 전용 앱은 후속 확장 대상
- On-device Analysis: Swift 기반 전경 픽셀 추출, K-means, CSV 최근접 감정 매핑, 감정 패밀리 집계
- Backend Prototype: Node.js 표준 HTTP 서버, 팔레트 휴리스틱 분석, Prometheus 메트릭
- ML/데이터 검증: Python, OpenCV, scikit-learn, CSV 데이터셋
- 테스트: Jest, Playwright, Python smoke/분석 스크립트
- 저장: 앱 프로필과 전시 카드는 `AppStorage`/`UserDefaults`; 서버 데이터베이스는 미구현
...
## 11. 마일스톤
- M1 — 완료: 문서 구조, 데이터셋, Python 기준선과 Node API 골격
- M2 — 완료: iPad 온보딩, 캔버스, 온디바이스 분석, 결과/3D 분포, 로컬 아카이브
- M3 — 다음: 감정 수정·메모, 개인 분포 학습 반영, 고급 색상 도구, 앱 테스트 자동화
- M4 — 후속: 인증·동기화·데이터베이스, 모델 전환 계약, iPhone 감상 앱, 배포 품질 고도화
