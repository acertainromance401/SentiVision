...
## 1. 제품 개요
SentiVision은 사용자가 그린 그림이나 업로드한 이미지의 색감을 분석해 감정의 결을 해석하고, 그 결과를 조용한 전시 형식으로 보여주는 감성 컴퓨팅 제품이다.

이 앱의 목적은 감정을 진단하거나 단정하는 것이 아니라, 말로 정리되지 않는 감정을 색과 구성으로 다시 바라보게 하는 데 있다.
...
## 2. 문제 정의
SentiVision이 해결하려는 핵심 문제는 "사용자가 창작 과정에서 느끼는 감정 상태를 부담 없이 기록하고, 색을 통해 새로운 관점으로 해석할 수 있는 도구가 부족하다"는 점이다.
...
6. 아카이브와 자기 인사이트 형성
- 사용자는 아카이브 화면에서 감정 제목, 팔레트, 메모를 다시 본다.
