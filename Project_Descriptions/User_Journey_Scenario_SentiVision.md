...
1. 사용자는 첫 실행에 프로필명, 기준 감정, 기준 색상, 체감 강도를 저장한다.
2. PencilKit 캔버스에서 기본 팔레트와 펜 굵기를 골라 그림을 그린다.
3. `Analyze Artwork`를 누르면 앱이 로컬에서 전경 픽셀을 추출하고 최대 3개 대표색과 감정을 계산한다.
4. 감정 해석, 신뢰도, 대표색, 감정 패밀리와 3D RGB 분포를 확인한다.
5. 결과를 전시 카드로 로컬 저장하고 아카이브에서 다시 선택해 본다.

감정 수정·메모, 개인 분포의 학습 반영, 고급 색상 입력, 원격 동기화는 아래 목표 여정에는 포함되지만 현재 백로그다.

### 단계 0. 개인 프로필 생성
- 사용자 행동: 첫 실행 시 이름, 기준 색상, 기준 감정을 고르고 체감 조절 수준을 선택한다.
- 현재 시스템 반응: 개인화 프로필을 로컬 저장하고 기준 감정을 결과 비교 문장에 사용한다.
- 후속 시스템 반응: 체감 강도를 분석 가중치에 반영하고 수정 이력을 개인 분포에 누적한다.
...
### 단계 3. 해석 요청
- 사용자 행동: 분석하기 버튼 탭
- 현재 시스템 반응: 앱이 로컬에서 전경 픽셀 추출 -> K-means -> CSV 최근접 감정 매핑을 수행한다.
- 후속 로딩 UX: 비동기 분석 또는 원격 모델 도입 시 단계형 진행 상태를 제공한다.
...
3. 사용자는 결과를 전시 카드로 저장한다.
4. 시스템은 로컬 아카이브에 반영하고, 사용자는 카드를 선택해 결과 화면을 다시 연다.
5. 후속 버전에서는 TRANQUILITY 등으로 감정을 수정하고 메모를 남긴다.
...
```mermaid
flowchart TB
  U[캔버스 이미지] --> A[iPad 온디바이스 분석]
  A --> P[전경 픽셀 -> K-means -> 최근접 감정]
  P --> R[감정 전시]
  R --> F[로컬 아카이브]
  F -. 후속 피드백 .-> D[개인 분포 보정]
  D --> C[CLI 검증]
