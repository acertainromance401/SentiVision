...
현재 저장소에는 동작 가능한 iPad 데모 앱, 독립 Node API, Python CLI 검증 파이프라인이 함께 구현되어 있다.
...
현재 구현 범위 (iPad 제품 데모)
- 첫 실행 프로필, 기준 색상·감정, 체감 강도 설정
- PencilKit 캔버스와 기본 팔레트, 펜·지우개·굵기 제어
- 캔버스 전경 픽셀 추출, 최대 3색 K-means, CSV 최근접 감정 예측
- 감정 패밀리 집계, 결과 해석, SceneKit 3D RGB 분포
- 전시 카드의 `UserDefaults` 저장과 재열람

현재 구현 범위 (독립 API/운영 프로토타입)
- Node.js `GET /health`, `GET /metrics`, `POST /analyze`
- 팔레트의 warmth·saturation·brightness 기반 휴리스틱 분석
- Jest/Playwright, 기능 플래그, A/B, Canary, Docker, 관측성 자산

현재 구현 범위 (Python 검증 엔진)
...
다음 구현 범위
- 고급 색상 선택(색상 휠, HEX/RGB, 스포이트, 팔레트 저장)
- 감정 수정과 메모 입력, 개인 분포 학습 반영
- 앱 분석과 원격 서비스 사이의 교체 가능한 계약
- 작품 이미지까지 포함하는 전시 카드와 상세 아카이브
...
### 2-2. API 레이어
- Node 프로토타입이 팔레트 분석, health, Prometheus metrics를 제공한다.
- 현재 iPad 앱은 이 API를 호출하지 않고 온디바이스 분석을 사용한다.
- 피드백 저장, 인증, 영속 데이터베이스, 원격 동기화는 후속 범위다.
...
```mermaid
flowchart LR
  U[사용자 드로잉] --> L[iPad 로컬 픽셀 추출]
  L --> K[K-means 대표색]
  K --> N[CSV 최근접 감정/패밀리]
  N --> R[전시 결과/로컬 아카이브]
  P[Python 연구 파이프라인] --> V[모델 비교/데이터 검증]
  A[Node 팔레트 API] --> O[메트릭/배포 검증]
```
...
- iPad 앱, 독립 Node API, Python 검증 트랙의 현재 경계를 함께 명시한다.
