...
문제 정의 정렬 메모
- 본 와이어플로우는 "그림을 마음껏 그리는 즐거움"을 먼저 제공하고, 그 색을 바탕으로 감정을 해석하는 흐름으로 구성한다.
...
| 화면 | 상태 | 비고 |
|---|---|---|
| 초기 세팅 | 구현 | 프로필, 기준 감정·색상, 체감 강도 |
| 환영 | 대체 구현 | 별도 홈 대신 프로필 배너와 Analysis/Canvas/Archive 탭 |
| 캔버스 | 구현/부분 | 기본 팔레트·펜·지우개·굵기 구현, 고급 색상 입력은 후속 |
| 분석 로딩 | 백로그 | 현재 로컬 동기 분석 |
| 결과 | 구현 | 해석·점수·대표색·패밀리·3D 분포와 저장 |
| 피드백 | 백로그 | 수정 감정·메모 입력 미구현 |
| 아카이브 | 구현/부분 | 로컬 카드 목록과 결과 재열람 구현 |
